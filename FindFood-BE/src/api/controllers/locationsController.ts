...
function centroidOf(coords: Array<{ lat: number; lng: number }>) {
...
export async function handleLocations(req: any, res: any) {
  try {
    ...
    const coords: Array<{ lat: number; lng: number }> = body.locations;
    if (coords.length === 0) return res.status(400).json({ error: 'empty locations' });
